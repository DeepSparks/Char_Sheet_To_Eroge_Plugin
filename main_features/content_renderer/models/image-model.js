...
class ImageModel extends BaseModel {
    constructor(attributes) {
        ...
        if(first_nsfw_keyword === 'none') {
            ...
            if(left_nsfw_keywords.length > 0) this.common_prompt = left_nsfw_keywords.join(', ') + ", " + this.common_prompt;
        } else if(FrontConfig.MAP_KEYWORDS.NSFW[first_nsfw_keyword]) {
            if(!FrontConfig.IS_NOT_USE_NSFW_PROMPT_MAP) {
                this.nsfw = FrontConfig.MAP_KEYWORDS.NSFW[first_nsfw_keyword];
                if(left_nsfw_keywords.length > 0) this.nsfw = this.nsfw + ", " + left_nsfw_keywords.join(', ');
            }
            this.common_prompt = FrontConfig.DEFAULT_KEYWORDS.NSFW.join(', ') + ", " + this.nsfw + ", " + this.common_prompt;
        } else {
            this.common_prompt = this.nsfw  + ", " + this.common_prompt;
        } 
        this.character_prompt = [this.pose, this.expression, this.etc_char].filter(attr => attr).join(', ') || '';
    }
