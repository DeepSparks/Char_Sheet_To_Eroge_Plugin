...
export default function restoreSceneTag(content) {
    const START_SCENE_TAG_REGEX = new RegExp(`<${FrontConfig.TAG_NAMES.SCENE}\-([^\\s>]+)(?:\\s+([^>]*))?>`, 'g')
    const END_SCENE_TAG_REGEX = new RegExp(`<\\/${FrontConfig.TAG_NAMES.SCENE}\-([^\\s>]+)>`, 'g')
    ...
    if(start_scene_matches.length > 0 && start_scene_matches.length !== end_scene_matches.length) {
        const tag_to_add = `</${FrontConfig.TAG_NAMES.SCENE}-${start_scene_matches[start_scene_matches.length - 1][1]}>`;
        if(content.includes(`</${FrontConfig.TAG_NAMES.SCENES}`)) {
            const scenes_tag_index = content.indexOf(`</${FrontConfig.TAG_NAMES.SCENES}>`);
